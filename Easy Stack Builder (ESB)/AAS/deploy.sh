...
cleanup() {
  set +e
  [ -f "$ROOT_DIR/Keycloak/values.yaml.bak" ] && mv "$ROOT_DIR/Keycloak/values.yaml.bak" "$ROOT_DIR/Keycloak/values.yaml"
  [ -f "$ROOT_DIR/Keycloak/realm/gaia-x-realm.json.bak" ] && mv "$ROOT_DIR/Keycloak/realm/gaia-x-realm.json.bak" "$ROOT_DIR/Keycloak/realm/gaia-x-realm.json"
  [ -f "$ROOT_DIR/AAS/values.yaml.bak" ] && mv "$ROOT_DIR/AAS/values.yaml.bak" "$ROOT_DIR/AAS/values.yaml"
...
sed -i.bak \
  ...
  "$ROOT_DIR/Keycloak/values.yaml"

sed -i.bak \
  -e 's/\<DOMAIN\>/'"$DOMAIN"'/g' \
  "$ROOT_DIR/Keycloak/realm/gaia-x-realm.json"

kubectl -n "$NAMESPACE" create configmap gaia-x-realm-config \
  --from-file=gaia-x-realm.json="$ROOT_DIR/Keycloak/realm/gaia-x-realm.json" \
  --dry-run=client -o yaml | kubectl apply -f -

log "Installing Keycloak"
helm dependency build "$ROOT_DIR/Keycloak"
helm upgrade --install keycloak "$ROOT_DIR/Keycloak" \
  --namespace "$NAMESPACE" \
  -f "$ROOT_DIR/Keycloak/values.yaml" \
...
sed -i.bak \
  -e 's/\<DOMAIN\>/'"$DOMAIN"'/g' \
  -e 's/\<TLS_SECRET\>/'"$TLS_SECRET"'/g' \
  "$ROOT_DIR/AAS/values.yaml"

JWK_SECRET="$(uuidgen 2>/dev/null || openssl rand -hex 16)"

helm dependency build "$ROOT_DIR/AAS" || true
helm upgrade --install auth-server "$ROOT_DIR/AAS" \
  --namespace "$NAMESPACE" \
  -f "$ROOT_DIR/AAS/values.yaml" \
