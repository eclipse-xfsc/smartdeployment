module.exports = function(RED) {
  ...
  function DeployNode(config) {
    ...
    const pcmname     = config.instanceName;
    const ocmname     = config.ocmInstanceName;
    ...
    const registryRepo = config.registryRepository;
    ...
    node.on('input', function(msg, send, done) {
      ...
      // Basic guardrails
      if (!pcmname || !ocmname || !domain || !crtContent || !keyContent || !kubeContent || !registryRepo || !registryUser || !registryPass ) {
      ...
      const args = [ pcmname, ocmname, domain, crtTmp.name, keyTmp.name, kubeTmp.name, registryRepo, registryUser, registryPass ];
    ...
    node.on('close', function(removed, done) {
      ...
      const args = [ pcmname, kubeTmp.name ];
